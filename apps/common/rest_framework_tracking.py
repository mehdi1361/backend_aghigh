...
class BaseLoggingMixin(object):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        if user.is_anonymous():
            user = None
        self.request.log['user_id'] = user.id
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        if self._should_log(request, response):
            ...
            try:
                self.request.log['response'] = json.dumps(response.data)
            except:
                self.request.log['response'] = json.dumps(response.data, default=json_util.default)
                # for loads
                # json.loads(aJsonString, object_hook=json_util.object_hook)

            try:
                send_message = threading.Thread(target=log_data_in_mongo, args=(self.request.log,))
                send_message.start()
...
def log_data_in_mongo(log):
    cursor.rest_log.insert_one(log)
